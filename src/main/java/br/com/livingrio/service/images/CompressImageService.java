package br.com.livingrio.service.images;
...
public class CompressImageService implements IService<Map<String,Object>>{

	public ServiceResponse execute(Map<String, Object> arg) {

		InputStream inputStream = (InputStream) arg.get("inputStream");
		
		int width = (Integer) arg.get("width");
		int height = (Integer) arg.get("height");
		
		BufferedImage result = resizeImage(inputStream, IMAGE_JPEG, width, height);
		...
		return new ServiceResponse(true, is);
	}
   ...
   */
  public static BufferedImage resizeImage(InputStream is, int type, int maxWidth, int maxHeight)
  {
    try {
      return resizeImage(ImageIO.read(is), type, maxWidth, maxHeight);
    }
    catch (IOException e) {
      e.printStackTrace();
    }
