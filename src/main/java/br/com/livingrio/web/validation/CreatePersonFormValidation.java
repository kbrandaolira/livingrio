...
public class CreatePersonFormValidation implements Validation<CreatePersonFormModel>{

	private PersonDAO personDAO;
	
	public CreatePersonFormValidation(PersonDAO personDAO) {
	...
	public List<ValidationMessage> validate(CreatePersonFormModel object) {
		...
		if(object.getPassword() == null || object.getPassword().equals("") || object.getPasswordConfirm() == null || object.getPasswordConfirm().equals("")){
			...
			if(!object.getPassword().equals(object.getPasswordConfirm())){
				...
				messages.add(new ValidationMessage("O campo confirmação de senha possui um valor diferente do valor do campo senha.", MessageType.ERROR));
